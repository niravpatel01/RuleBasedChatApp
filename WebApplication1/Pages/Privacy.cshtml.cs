...
namespace WebApplication1.Pages
{
    public class PrivacyModel : PageModel
    {
        ...
        private List<string> SearchIssues22(string question)
        {
            var results = new List<string>();
            ...
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "data", "issues.xlsx");
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                {
                    string issueText = worksheet.Cells[row, 1].Text.Trim(); // Adjust column index if needed

                    if (words.Any(word => issueText.ToLower().Contains(word)))
                    {
                        results.Add(issueText);
                    }
                }
            }
        ...
        private List<string> SearchIssues2(string question)
        {
            var allIssues = new List<string>
    {
        ...
        "Technical Errors"
            ...
            return allIssues
                .Where(issue => issue.Contains(question, StringComparison.OrdinalIgnoreCase))
                .ToList();
        }
